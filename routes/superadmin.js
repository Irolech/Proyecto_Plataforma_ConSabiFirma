...
// =====================================================================
// 0. INICIALIZACIÓN DE INFRAESTRUCTURA DE FECHAS
// =====================================================================
db.run(`
...
// =====================================================================
// 1. VISTA: DASHBOARD PRINCIPAL DEL SUPERADMIN (GET)
// =====================================================================
router.get('/dashboard/:dni', (req, res) => {
    const adminDni = req.params.dni;

    // Control de seguridad estricto por base de datos
                    ...
                    <body>
                        <div style="padding: 40px; max-width: 1300px; margin: 0 auto;">
                            ...
                            <header style="display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom: 40px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 20px; gap: 20px;">
                                ...
                                <div style="margin-left: auto;">
                                    <a href="/admin/${superuser.dni}" class="btn btn-outline" style="color: #f8fafc; border-color: rgba(255,255,255,0.3); text-decoration: none; padding: 10px 20px; border: 1px solid; border-radius: 6px; display: inline-block; white-space: nowrap;">Ir a la aplicación de firma</a>
                                </div>
                            </header>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <form action="/superadmin/usuarios/eliminar" method="POST" style="display:inline;">
                                                                <input type="hidden" name="currentAdminDni" value="${superuser.dni}">
                                ...
                                <div>
                                    ...
                                    <div class="card-super" style="border: 1px solid var(--super-blue);">
                                        ...
                                        <form action="/superadmin/traspaso-voluntario" method="POST" style="margin-top: 15px;">
                                            ...
                                            <label>DNI del Sucesor Administrador:</label>
                                            <input type="text" name="nuevoDni" required placeholder="87654321B">
                        ...
                        <script>
                            ...
                            function cargarFormularioEdicion(dni, nombre, apellidos, email, cargo, rol) {
                                ...
                                // Si por alguna razón es superadmin, seleccionará el primero por defecto (protección visual)
...
// =====================================================================
// 2. ACCIÓN: CREAR NUEVO USUARIO (POST + BLINDAJE)
// =====================================================================
router.post('/usuarios/crear', (req, res) => {
    const { currentAdminDni, dni, nombre, apellidos, email, cargo, password, rol } = req.body;

    // 🛡️ Ajuste extra de seguridad: Bloqueo radical de inyecciones de privilegios
    if (rol === 'superadmin') {
    ...
    db.run(query, [dni.trim().toUpperCase(), nombre.trim(), apellidos.trim(), email.trim(), cargo.trim(), password, rol], function (err) {
        ...
        res.send(`<script>alert("✅ Usuario creado con éxito en el sistema."); window.location.href = "/superadmin/dashboard/${currentAdminDni}";</script>`);
    });
});

// =====================================================================
// 3. ACCIÓN: EDITAR USUARIO EXISTENTE (POST + BLINDAJE)
// =====================================================================
router.post('/usuarios/editar', (req, res) => {
    const { currentAdminDni, dni, nombre, apellidos, email, cargo, rol } = req.body;
    ...
    db.run(query, [nombre.trim(), apellidos.trim(), email.trim(), cargo.trim(), rol, dni], function (err) {
        ...
        res.send(`<script>alert("✅ Registro actualizado correctamente."); window.location.href = "/superadmin/dashboard/${currentAdminDni}";</script>`);
    });
});
...
router.post('/usuarios/eliminar', (req, res) => {
    const { currentAdminDni, dniEliminar } = req.body;
    ...
    db.run("DELETE FROM usuarios WHERE dni = ?", [dniEliminar], function (err) {
        ...
        res.send(`<script>alert("🗑️ Usuario eliminado de forma definitiva."); window.location.href = "/superadmin/dashboard/${currentAdminDni}";</script>`);
    });
});
...
// =====================================================================
router.post('/traspaso-voluntario', (req, res) => {
    const { currentAdminDni, nuevoDni, tipoTraspaso, fechaHoraEfecto } = req.body;
    const dniSucesor = nuevoDni.trim().toUpperCase();
        ...
        if (tipoTraspaso === 'inmediato') {
            db.serialize(() => {
                ...
                db.run("INSERT INTO auditoria (usuario_dni, accion, detalles) VALUES (?, 'TRASPASO DIRECTO', ?)",
                    [currentAdminDni, `Cede voluntariamente los poderes a ${usuarioDestino.nombre} ${usuarioDestino.apellidos}.`]);

                res.send(`
                    <script>
                        alert("👑 Traspaso completado con éxito. Tus privilegios globales expiran ahora mismo. Serás redirigido.");
            ...
            const queryInsert = `INSERT INTO cambios_superadmin_programados (dni_nuevo, dni_antiguo, fecha_ejecucion) VALUES (?, ?, ?)`;

            db.run(queryInsert, [dniSucesor, currentAdminDni, fechaHoraEfecto], function (errInsert) {
                if (errInsert) {
                ...
                db.run("INSERT INTO auditoria (usuario_dni, accion, detalles) VALUES (?, 'TRASPASO PROGRAMADO', ?)",
                    [currentAdminDni, `Agenda cambio de superadmin para el día/hora: ${fechaHoraEfecto.replace('T', ' ')}.`]);

                res.send(`<script>alert("📅 Traspaso agendado para el ${fechaHoraEfecto.replace('T', ' ')}. Hasta entonces seguirás al mando."); window.location.href = "/superadmin/dashboard/${currentAdminDni}";</script>`);
            });
        }
    });
});
...
// =====================================================================
// 6. ACCIÓN: REEMPLAZO POR LLAVE MAESTRA OTP (MANTENIDO COMO BYPASS)
// =====================================================================
router.post('/reemplazar', (req, res) => {
    ...
    db.get("SELECT * FROM llaves_maestras WHERE clave_hash = ? AND utilizada = 0", [hashIntroducido], (err, llaveValida) => {
        if (err || !llaveValida) {
            return res.send(`<script>alert("❌ Llave maestra inválida o quemada."); window.history.back();</script>`);
        }

        db.get("SELECT nombre, apellidos FROM usuarios WHERE dni = ?", [nuevoDni.trim()], (errUser, usuarioDestino) => {
            if (errUser || !usuarioDestino) {
                return res.send(`<script>alert("❌ El DNI de destino no existe."); window.history.back();</script>`);
            }

            db.serialize(() => {
                const ahora = new Date().toISOString();
                db.run("UPDATE llaves_maestras SET utilizada = 1, fecha_uso = ? WHERE id = ?", [ahora, llaveValida.id]);
                db.run("UPDATE usuarios SET rol = 'usuario' WHERE rol = 'superadmin'");
                db.run("UPDATE usuarios SET rol = 'superadmin' WHERE dni = ?", [nuevoDni.trim()]);
                db.run("INSERT INTO auditoria (usuario_dni, accion, detalles) VALUES (?, 'BÚNKER: REEMPLAZO OTP', ?)",
                    [nuevoDni.trim(), `Poderes asumidos de emergencia usando la Llave Maestra #${llaveValida.id}.`]);

                res.send(`<script>alert("👑 ¡Protocolo Ejecutado! Nuevo superadmin asignado."); window.location.href = "/";</script>`);
            });
        });
    });
});
